...
export default function StockTickerManager() {
  ...
  const [include_fomc, setIncludeFOMC] = useState(false);
  const [include_bls, setIncludeBLS] = useState(false);
  const [include_bea, setIncludeBEA] = useState(false);
  ...
  const generateCalendar = async () => {
    if (tickers.length === 0 && !include_fomc && !include_bls && !include_bea) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          {/* Ticker Count */}
          {(tickers.length > 0 || include_fomc || include_bls || include_bea) && (
            <div className="pt-6 border-t border-gray-200">
              <div className="flex items-center justify-between mb-4">
                <div className="text-gray-600 text-sm">
                  ...
                  {(include_fomc || include_bls || include_bea) && (
                    <p className="mt-1">
                      Economic calendars: {[
                        include_fomc && 'FOMC',
                        include_bls && 'BLS',
